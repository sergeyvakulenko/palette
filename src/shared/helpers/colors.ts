...
import { FlatPalette, Palette, Color, FlatColor, Format } from "../types";
  ...
  for (const color of starterPallete.colors) {
    ...
    for (const i in scale) {
      const newColor: Color = {
        ...
        rgba: chroma(scale[i])
          .css()
          .replace(Format.RGB, Format.RGBa)
          .replace(")", ",1.0)"),
      };
      newPallete.colors[LEVELS[i]].push(newColor);
    }
  }
  return newPallete;
}

function getRange(hex: string): string[] {
  const end = "#fff";
  return [chroma(hex).darken(1.4).hex(), hex, end];
}

function generateScale(hex: string, num: number): string[] {
...
export function getRandomColor(): FlatColor {
  const allColors = defaultPalettes().reduce(
    (acc: FlatColor[], val: FlatPalette) => acc.concat(val.colors),
    []
  );
...
export function defaultColors(): FlatColor[] {
  return defaultPalettes()[0].colors;
}

export function defaultPalettes(): FlatPalette[] {
  return [
    {
